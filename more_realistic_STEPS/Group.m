...
%             self.grid_coords = [randsample(grid_init,self.number_of_nodes,1)' randsample(grid_init,self.number_of_nodes,1)']; % equi-distanced zones
            ...
            for i=1:length(node_config)
                self.beta = sum((1 + (0:self.d_max)).^(-alpha(i)))^-1;
                aux = self.beta*((0:self.d_max)' + 1).^(-alpha(i));
        ...
        function move(self)
            ...
            in_continue_moving = in_way & d_move < d_to_next_wp;
            
            % % % % reached the Waypoint
            in_arrived = in_way & d_move >= d_to_next_wp;
            self.coords(in_arrived,:) = self.next_waypoint(in_arrived,:);
...
%                 in_no_pause = in_arrived & self.pause_time==0;
%                 d_to_next_wp(in_no_pause) = sqrt(sum((self.next_waypoint(in_no_pause,:)-self.coords(in_no_pause,:)).^2,2));% 20/11/2013
%                 in_continue_moving = (in_way & d_move < d_to_next_wp) | in_no_pause;
            end
            
            % % % % not yet reached the Waypoint          
            if ~isequal(in_continue_moving, zeros(self.number_of_nodes,1))
                aux = self.next_waypoint(in_continue_moving,:) - self.coords(in_continue_moving,:);
                aux1 = [aux(:,1).*self.time_step.*self.speed(in_continue_moving)./d_to_next_wp(in_continue_moving) aux(:,2).*self.time_step.*self.speed(in_continue_moving)./d_to_next_wp(in_continue_moving)];
                self.coords(in_continue_moving,:) = self.coords(in_continue_moving,:) + aux1;
            end
        ...
        %------------------------------------------------------------------        
        function [connection_matrix,D1] = connect(self)
...
%             D = min(D1, min(D2, min(D3,D4))); % for a torus area
            D = D1; % for a bounded area
